...
export function Home() {
    ...
    const [mealsPerView, setMealsPerView] = useState(0);
    const [dessertsPerView, setDessertsPerView] = useState(0);
    const [drinksPerView, setDrinksPerView] = useState(0);
    ...
    useEffect(() => {
        ...
        .then((response) => {
            ...
            const mealAmount = response.data.filter((meal) => meal.category === "Refeições").length;
            const dessertAmount = response.data.filter((dessert) => dessert.category === "Sobremesas").length;
            const drinksAmount = response.data.filter((drink) => drink.category === "Bebidas").length;

            if(mealAmount < 4) {
                setMealsPerView(mealAmount);
            }else {
                setMealsPerView(3.8);
            }
            if(dessertAmount < 4) {
                setDessertsPerView(dessertAmount);
            }else {
                setDessertsPerView(3.8);
            }
            if(drinksAmount < 4) {
                setDrinksPerView(drinksAmount);
            }else {
                setDrinksPerView(3.8);
            }
        })
        .catch(error => {
            if(error.response) {
